...
Hero on the left (eyebrow, title, one-sentence lead, **Start exploring** teal link, **Watch a worked example** plain link, "no setup" note). On the right, the landing figure: the same panel unshaded (dotted) and shaded (teal, three peaks, amber dot on the true one) — drawn live by `module_iv`, not an image. Below: "What you can do here", five cards, each linking to its page.
