...
| 3 | S5 | Array generalisation | Single module is the N=1 special case (reproduces module_iv exactly); N identical modules scale V and P by N at constant I; modules at differing irradiance give a string-level multi-peak curve via module-level bypass — the S3 substring-composition mechanism reused one level up. Standalone (see note). **DONE.** |
...
- `s5_string_multipeak.png` — string of 3 modules in series; a shaded string is multi-peak by the same mechanism as a shaded module.
