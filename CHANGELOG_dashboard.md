...
## Global theme — Dark by default, the user's choice authoritative, one source per session (2026-09-25)

Two root causes of "the page turns light after a rerun", neither in the collapsible cards:
1. **The theme pill could be deselected.** `st.segmented_control` clears its value when the active pill is clicked, leaving `gm_theme = None`; `setup(None)` resolves to light, and because the key then exists `setdefault` never restored Dark, so every later rerun stayed light. Now: `gm_theme_pref` holds the last explicit choice (default "Dark"); at the top of every run a `gm_theme` that is not "Light"/"Dark" is set back to the preference. Dark is a default, never rewritten; an explicit Light or Dark is kept until the pill is used again.
2. **The resolved palette was a process-wide global** (`gmppt_ui._theme_name`), shared by every session and thread, so two sessions on different themes raced: one run could inject dark CSS in `setup()` and read the light palette from `ui.T()` later in the same run. `setup()` now also stores the name in `st.session_state["_gm_theme_name"]` and `T()` reads that first; the global remains only as the bare-mode fallback for tests.
- One key everywhere (`gm_theme`, the header's control on every page and the landing top bar); no page or card has a theme default of its own.
- Verified (Chrome, 35/35): fresh session Dark; Dark survives open/close of each of the three cards, module change, shape, severity and position sliders, topology, panel selection, optimiser toggle, Send, navigating away and back; clicking the active Dark pill leaves Dark; explicit Light survives the same list; explicit Dark again survives navigation. `tests/test_theme.py` (AppTest): fresh session Dark; `gm_theme = None` restores the previous choice in both themes; an explicit choice persists across reruns.

## Panel type — the whole research validation set, one source (2026-09-25)

Backend only. `_validation_modules()` no longer hand-picks three modules by cell-count/power bands: it lists every module of `gmppt.dataset.module_split().val` that the CEC pool carries (3,351), resolved through the existing `_val_module_names()` — the same call `phase2/p7_tracker_comparison.py --split val` makes before `scenarios_for_modules(split.val, n)` (phase3/p3_final_comparison.py) keeps only those modules. Labels are `<CEC name> · <cells>-cell · <STC W>`; the name is the CEC record `_mparams()` resolves, so the chosen module is the module the validated engine runs. The "Nanum target panel — [awaiting spec]" placeholder, which stood in a different module, is gone; a stored choice not in the list resets explicitly to the documented demo module (a validation module by construction), never to "whatever is first". No physics, state, hash, tracker or UI change.
- Verified (Chrome, 23/23): default = LG370S2W_A5; A → AU_Optronics_PM245PA2_240 changes both hashes and the physics (V_oc 44.0 → 34.8 V, 313 → 201 W) with the no-shadow reference following; Send → ● sent; A → C (Photowatt PW2750_280) without sending → ● changed since sent; Static test runs the SENT module (hybrid seed 27.3 V, inside B's 34.8 V range, below A's 35.6 V peak); Send again → the tracker follows C (seed 32.7 V, inside 40.8 V). Unit: the helper executed against the real split and pool returns exactly the validation modules present, sorted, unique labels; the tracker page runs `scenario_sent["module"]` with no substitute; the demo module's numbers are unchanged (144.56 W at 24.25 V, 3 peaks).

## Scenario centre — the panel as the dominant object (2026-09-25)

Presentation only, centre column of Scenario · Build system. Physics, states, hashes, draft/sent and every other card untouched.
- The two-column centre (small panel left, A/B/C text right) is one centred vertical stack: panel drawing (`module_face_svg`, now 280 px wide, +47 %, responsive through `width: min(100%, 280px)` on a fixed 200 × 300 viewBox, in-SVG type set smaller so it stays modest) → a three-cell **section status strip** (`section_strip_html`: one equal cell per section, Working / Bypassed / Partly bypassed from the same `_diode_state` results the page already held, padded to line up with the sections drawn above, amber when a bypass path is active, lab suffix "bypass on/off") → **Panel open-circuit N V** on its own line (lab: V_oc) → the generated sentence ("Section B is bypassed because of the shadow, so current goes around it at the real peak. The panel now has 2 possible power peaks.").
- Multi-panel keeps the hierarchy: topology overview above, one large detail of the selected panel below.
- Verified (Chrome, 24/24): 280 px, centred to 0.5 px, old block gone, three cells 73 × 56 each under their own section, Pole at 50 % → Working / Bypassed / Working with the sentence and the Peaks KPI (2) from the same state, lab wording, light theme, 3S × 1P topology above one detail, 820 px without overflow. `tests/test_layout.py::test_centre_is_one_vertical_panel_stack` added; the page-render suite passes.
