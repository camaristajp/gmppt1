...
| **R3 (banner)** | `ui.scenario_banner` under the header on every Explore/Testing page: module · T · pattern · geometry · split · engine, with a chip reading `sent`, `edited since sent` + **Resend**, `example scenario`, `aggregate page` or `fixed demo module`. Rendered into a placeholder *after* the body so it can never lag a render behind. | `ui.scenario_banner`, `_scenario_bar`, `_make_page` | T4 | None. |
