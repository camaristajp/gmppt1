...
def render_setup():
    ...
    e1, e2 = st.columns(2)
    e1.number_input("Base irradiance [W/m²]", 100, 1200, key="base_val", step=25,
                    help="Full sunlight before shading. STC = 1000 W/m².")
    e2.number_input("Cell temperature [°C]", -10, 70, key="temp_c", step=1,
...
def render_results():
    ...
    res, m = r["res"], r["m"]
    # action bar
    ab = st.columns([3, 1, 1, 1])
    ab[0].markdown("#### ③ Simulation result")
    if ab[1].button("💾 Save", use_container_width=True, help="Save scenario for comparison"):
    ...
    if ab[2].button("⇄ Compare", use_container_width=True):
...
def render_simulator():
    with st.sidebar:
        ...
        st.caption(f"{SHADING_ICON.get(st.session_state.shade_object,'')} "
                   f"{st.session_state.shade_object.split(' / ')[0]} · "
                   f"{st.session_state.shade_severity}")
...
def main():
    ...
    init_state()
    global TH
    TH = THEMES["Light" if st.session_state.theme_choice.startswith("🌞") else "Dark"]
    apply_theme_css(TH)
...
def render_analysis():
    ...
    with tabs[0]:
        if have:
            ...
            _empty_state("Run a simulation first (Simulator → Scenario → Run).")
    with tabs[1]:
        if have:
            res = r["res"]
            brows = [{"Substring": f"S{i+1}", "Irradiance": f"{int(g)} W/m²",
            ...
            _empty_state("Run a simulation to inspect substring behavior.")
    with tabs[2]:
        if have:
            ...
            _empty_state("Run a simulation to see detected peaks.")
    with tabs[3]:
        render_sweep()
    with tabs[4]:
        render_efficiency()
